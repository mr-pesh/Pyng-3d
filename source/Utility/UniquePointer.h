...
{
    template <class ...Args>
    FORCE_INLINE_S T* Create(Args ...args) noexcept (std::is_nothrow_constructible<T>::value)
    {
        return new T(args...);
    }
    FORCE_INLINE_S void Destroy(T *ptr) noexcept (std::is_nothrow_destructible<T>::value)
    {
        delete ptr;
    }
};
...
struct StackAllocationPolicy
{
    ...
    FORCE_INLINE_S void Destroy(T *ptr) noexcept
    {
    #ifdef _MSC_VER
        _freea((void*)ptr);
    #else
